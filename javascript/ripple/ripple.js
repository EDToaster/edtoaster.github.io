...
function Ripple(canvas) {
    ...
    this.timer = null;
    this.lasttick = Ripple.now();
    this.fps = 0;

    gl.disable(gl.DEPTH_TEST);
    this.programs = {
        copy: igloo.program('glsl/quad.vert', 'glsl/copy.frag'),
        Ripple:  igloo.program('glsl/quad.vert', 'glsl/Ripple.frag')
    };
    this.buffers = {
 ...
 * @returns {Ripple} this
 */
Ripple.prototype.set = function(state) {
 ...
 * @returns {Ripple} this
 */
Ripple.prototype.setRandom = function(p) {
 ...
 * @returns {Ripple} this
 */
Ripple.prototype.setEmpty = function() {
 ...
 * @returns {Ripple} this
 */
Ripple.prototype.swap = function() {
 ...
 * @returns {Ripple} this
 */
Ripple.prototype.poke = function(x, y, state) {
    var gl = this.igloo.gl,
        v = state * 255;
 ...
 */
Ripple.prototype.eventCoord = function(event) {
 ...
 */
function Controller(Ripple) {
    this.Ripple = Ripple;
    var _this = this,
        $canvas = $(Ripple.igloo.canvas);
    this.drag = null;
    $canvas.on('mousedown', function(event) {
        _this.drag = event.which;
        var pos = Ripple.eventCoord(event);
        Ripple.poke(pos[0], pos[1], _this.drag == 1);
        Ripple.draw();
    });
    ...
    $(document).on('keyup', function(event) {
        ...
        case 83: /* s */
            if (event.shiftKey) {
                if (this._save) Ripple.set(this._save);
            } else {
                this._save = Ripple.get();
            }
            break;
        };
    });
...
var ripple = null, controller = null;
$(document).ready(function() {
    var $canvas = $('#life');
    ripple = new Ripple($canvas[0]).draw().start();
    controller = new Controller(ripple);
});
